...
- [x] FR-004: 섹션 순서: 자기소개 → 경력 → 학력 → 스킬 → 자격증 → 프로젝트 → 포트폴리오 (빈 섹션 생략)
...
| Method | Path | 응답 | 설명 |
|--------|------|------|------|
| GET | `/resume/api/v1/view/pdf` | `application/pdf` | 내 기본 이력서 PDF (Content-Disposition: attachment; filename*=UTF-8''%EC%9D%B4%EB%A0%A5%EC%84%9C.pdf) |
...
- `ResumePdfServiceImplTest`:
  - 전체 항목 포함 view → `%PDF` 헤더 + 길이 > 1KB
