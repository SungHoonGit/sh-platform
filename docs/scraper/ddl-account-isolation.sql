...
ALTER TABLE crawl_config
    ADD COLUMN account_id BIGINT NOT NULL DEFAULT 1 COMMENT '소유 계정 ID (auth users.id)' AFTER id;

-- 2. 전역 이름 유니크 → 계정별 이름 유니크로 변경 (다중 사용자 시 동일 이름 허용)
ALTER TABLE crawl_config
    DROP INDEX uk_crawl_config_name,
    ADD UNIQUE KEY uk_crawl_config_account_name (account_id, name);
