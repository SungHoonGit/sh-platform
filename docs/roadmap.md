...
## Phase 5: SaaS 테넌트 관리 (🔜 진행 예정)

### 5.1 기본 테넌트 관리

- [ ] DB 스키마 생성 (sh_tenant, sh_tenant_member, sh_tenant_invitation)
- [ ] 테넌트 CRUD API (생성/조회/수정/삭제)
- [ ] 테넌트 컨텍스트 관리 (TenantContext + Interceptor)
- [ ] 테넌트-사용자 관계 (테넌트 생성 시 자동 소유)

### 5.2 멤버 관리

- [ ] 멤버 초대 API (이메일 초대, 토큰 발급)
- [ ] 멤버 수락 API (초대 수락, 역할 설정)
- [ ] 멤버 역할 변경 API (ADMIN/MEMBER/GUEST)
- [ ] 멤버 제거 API

### 5.3 데이터 격리

- [ ] 테넌트 ID 기반 필터링 (모든 쿼리에 적용)
- [ ] 권한 검증 (테넌트 멤버만 접근 가능)
- [ ] 격리 테스트 (테넌트 간 데이터 격리 확인)

### 5.4 테넌트 설정

- [ ] 테넌트별 설정 관리 (JSON 설정 저장)
- [ ] 설정 조회/수정 API
- [ ] 테넌트 상태 관리 (ACTIVE/SUSPENDED/DELETED)

### 5.5 감사 로그

- [ ] 테넌트 감사 로그 기록 (sh_tenant_audit_log)
- [ ] 감사 로그 조회 API
...
- [ ] 테넌트 생성/조회/수정/삭제 정상 동작
- [ ] 멤버 초대/수락/역할 변경 정상 동작
- [ ] 테넌트 간 데이터 격리 확인
- [ ] API 문서 (Swagger) 갱신
- [ ] Javadoc 작성
- [ ] 단위 테스트 작성
