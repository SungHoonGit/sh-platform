...
> **⚠️ 진행 중인 작업**: 이력서 아이템 문서(documentId) 단위 분리 구현·배포 완료, 실사용 검증 예정.
> **세션 시작 시 반드시 `docs/daily/2026-09-13-work-log.md`를 먼저 읽고 이어서 작업할 것.**
> **세션 시작 시 반드시 `git pull origin master`로 원격과 동기화 + 오늘 날짜의 작업 일지(`docs/daily/YYYY-MM-DD-work-log.md`)를 먼저 읽고 이어서 작업할 것.**
> 최근 완료: 공고↔이력서 기술 매칭(신규) + 이력서 아이템 문서 단위 분리·documentId 마이그레이션(DDL v13)·DB binlog 활성화·403 자동 토큰 갱신·제목 인라인 수정·displayOrder 보존.
> 다음: 실사용 검증(문서별 독립성, 불러오기 복제) + 주기 백업(mysqldump) 스케줄 추가(승인 대기).
