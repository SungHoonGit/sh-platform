...
> **⚠️ 진행 중인 작업**: 이력서 PDF — 테마별(Classic/Modern/Saramin) 레이아웃 + 파일명(테마+제목)+페이지 분리+사진 통일 구현 완료. 커밋/푸시 후 서버 배포 확인 대기.
> **세션 시작 시 반드시 `docs/daily/2026-08-31-work-log.md`를 먼저 읽고 이어서 작업할 것.**
> 최근 완료: 노트블랙리스트 카테고리별 차단 통계(6515184), 학교/전공 마스터 확장(056420b), Redis 서버 검증(5832bb2), 이력서 PDF(023 설계)·테마 레이아웃(20ed19d).
> 다음: 개선분(파일명/페이지 분리/사진 통일) 배포 확인 + 사용자 검증.
