...
> **⚠️ 진행 중인 작업**: 기술 스택 키워드화 + 유사검색(`5c7b17a`) 구현·배포 완료, 실사용 검증 예정.
> **세션 시작 시 반드시 `docs/daily/2026-09-09-work-log.md`를 먼저 읽고 이어서 작업할 것.**
> **세션 시작 시 반드시 `git pull origin master`로 원격과 동기화 + 오늘 날짜의 작업 일지(`docs/daily/YYYY-MM-DD-work-log.md`)를 먼저 읽고 이어서 작업할 것.**
> 최근 완료: 포트폴리오 관리(`11f2abb`) + 명칭 변경·itemType 수정(`65a2b73`) + 경력 기간별 상세·공통 필드 표준화(`d1df3b0`) + 기술 스택 키워드화·유사검색(`5c7b17a`).
> 다음: 실사용 검증(경력 상세 등록 → 이력서 보기/PDF/공유, 작업물 → 프로젝트 가져오기 1:1, 프로젝트 썸네일, 기술 스택 자동완성).
