...
> 최근 완료: 이력서 아이템 문서 단위 분리(documentId, DDL v13) + 403 자동 토큰 갱신 + 제목 인라인 수정 + displayOrder 보존 + **/resume 404 복구(배포 jar에 프론트 dist 누락 → 재배포)** + **MySQL 주기 전체 백업(매일 03:30 KST mysqldump, 7일 보존, cron.d)**.
> 다음: 실사용 검증(문서별 독립성, 불러오기 복제) + 백업 1회 실동작 확인(내일 03:30 KST 산출물 확인).
>
