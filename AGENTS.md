...
> **⚠️ 진행 중인 작업**: 공고↔이력서 기술 매칭(`/match-skills`) 구현·배포 완료, 실사용 검증 예정.
> **세션 시작 시 반드시 `docs/daily/2026-09-10-work-log.md`를 먼저 읽고 이어서 작업할 것.**
> **세션 시작 시 반드시 `git pull origin master`로 원격과 동기화 + 오늘 날짜의 작업 일지(`docs/daily/YYYY-MM-DD-work-log.md`)를 먼저 읽고 이어서 작업할 것.**
> 최근 완료: 기술 스택 키워드화·유사검색(`5c7b17a`, 935299b) + 공고↔이력서 기술 매칭(신규).
> 다음: 실사용 검증(지원 관리 스크랩 → 일치 기술 배지, 기술 스택 자동완성, 경력 상세/작업물 가져오기 외).
