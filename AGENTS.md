...
> **⚠️ 진행 중인 작업**: 포트폴리오 관리 — 독립 페이지 + 썸네일/링크 필드(DDL v10) + 이력서 프로젝트 "작업물에서 가져오기"(복사) + 보기/PDF/공유 카드 구현·배포 완료, 실사용 검증 예정.
...
> 최근 완료: 데이터 섹션·문서 목록 순번 관리(`11f2abb` 직전) + 포트폴리오 관리 기능(`11f2abb`).
> 다음: 포트폴리오 실사용 검증(작업물 등록 → 독립 페이지 → 이력서 프로젝트 가져오기 → 보기/PDF/공유 썸네일).
