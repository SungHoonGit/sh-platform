...
> **⚠️ 진행 중인 작업**: 데이터 섹션 순번 관리 — reorder API(7종) + 드래그/▲▼ UI + 인라인 목록 상시 표시 구현 완료, 커밋/푸시 후 CI 배포 확인 예정.
...
> 최근 완료: 계정 설정 비밀번호 오류 UX 개선(`e7b313e`) + 데이터 섹션 순번 관리(본 세션).
> 다음: CI 배포 확인 후 reorder 동작 실사용 검증.
