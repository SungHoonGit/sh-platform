...
> **⚠️ 진행 중인 작업**: 경력 기간별 상세 문서화 + 작업물↔프로젝트 공통 필드 표준화(`d1df3b0`) 구현·배포 완료, 실사용 검증 예정.
...
> 최근 완료: 포트폴리오 관리(`11f2abb`) + 명칭 변경·itemType 수정(`65a2b73`) + 경력 기간별 상세·공통 필드 표준화(`d1df3b0`).
> 다음: 실사용 검증(경력 상세 등록 → 이력서 보기/PDF/공유, 작업물 → 프로젝트 가져오기 1:1, 프로젝트 썸네일).
