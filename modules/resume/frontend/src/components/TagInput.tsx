...
/**
 * Notion 스타일 태그 입력기. 마스터 검색 제안 + Enter/선택으로 추가, x로 제거.
 * 저장 시점은 부모가 결정한다 (스크래퍼 TagInput과 동일한 동작, resume 전용 복사).
 */
  ...
  return (
    <div className="relative">
      ...
      {showSuggest && suggestions.length > 0 && (
        <ul className="absolute z-10 mt-1 max-h-44 w-full overflow-auto rounded-lg border border-gray-200 bg-white shadow-lg">
          {suggestions.map((s) => (
            <li key={s.id ?? s.name}>
              <button
                type="button"
